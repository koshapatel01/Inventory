...
function getClient() {
  if (!client) {
    ...
    // Next.js patches the global `fetch` that this driver's HTTP-based
    // queries use under the hood, and applies its own persistent Data Cache
    // to it independently of a route's `dynamic` export — a page marked
    // `force-dynamic` skips the CDN/route cache, but a fetch() call made
    // deep inside a dependency can still get memoized by the Data Cache the
    // first time it runs, then keep serving that same stale result on every
    // later request (observed in production: /orders kept rendering the
    // exact order count from whenever its query was first cached, while the
    // /api/orders route handler — evidently not hitting the same cache path
    // — reflected live data). Forcing no-store here, once, for every query
    // this client ever makes is more reliable than trying to opt out at
    // each call site.
