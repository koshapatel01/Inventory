...
// pdfjs-dist runs without a real Worker thread on Node (it disables that
// path itself when isNodeJS is true) and instead needs the worker module's
// exports in-process. Its own fallback for that — `_setupFakeWorkerGlobal`
// in pdf.mjs — does `await import(this.workerSrc)` with a *computed*
// specifier (a string built from GlobalWorkerOptions.workerSrc, not a
// literal), which Vercel's build-time file tracer can't statically resolve,
// so pdf.worker.mjs never makes it into the deployed function bundle even
// though it exists locally — the exact same class of bug as the
// @napi-rs/canvas import above. pdfjs-dist checks globalThis.pdfjsWorker
// first and skips that dynamic import entirely if it's already set, so
// importing the worker module ourselves, via a plain static import Vercel's
// tracer CAN follow, avoids the gap altogether.
