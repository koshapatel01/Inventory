...
//
// Requires at least one digit AND one letter somewhere in a 6-20 char
// uppercase/digit run, rather than requiring the digit to appear near the
// front — real SKUs like "DURAACTBULK36" (a bulk-pack code with its only
// digits at the very end) were silently invisible to an earlier version of
// this pattern that only looked within the first ~7 characters, which made
// an entire line item (and sometimes the whole invoice) vanish from parsing
// whenever that SKU wasn't already in the catalog. The 6-char minimum keeps
// this from matching short glued-together description fragments like
// "36PK" (from "AA,36PK" in a comma-separated description) — no real SKU in
// this app's catalog is shorter than 6 characters.
const GENERIC_SKU_PATTERN = /\b(?=[A-Z0-9]*\d)(?=[A-Z0-9]*[A-Z])[A-Z0-9]{6,20}\b/g;
